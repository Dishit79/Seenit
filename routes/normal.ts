...
$("body > main > div.container.mt-2 > div > div.w3-col.s12.mt-1").find("li").each(function(i, link){
  gatheredInfo.push({name: $(link).find('h5 > a').text(), type: $(link).find('.category').text(), size: $(link).find('img[alt="Size"]').parent("div").text(), magnetLink: $(link).find('.dl-magnet').attr('href')})
})
